...
fn modifier_from_digits(first: char, second: Option<char>) -> Option<M> {
    let code = match second {
        Some(next) => (first.to_digit(10)? * 10 + next.to_digit(10)?) as u8,
        None => first.to_digit(10)? as u8,
    };
    match code {
        2 => Some(M::SHIFT),
        3 => Some(M::ALT),
        ...
        6 => Some(M::CTRL_SHIFT),
        7 => Some(M::CTRL_ALT),
        8 => Some(M::CTRL_ALT_SHIFT),
        9 => Some(M::SUPER),
        10 => Some(M::SUPER_SHIFT),
        11 => Some(M::SUPER_ALT),
        12 => Some(M::SUPER_ALT_SHIFT),
        13 => Some(M::SUPER_CTRL),
        14 => Some(M::SUPER_CTRL_SHIFT),
        15 => Some(M::SUPER_CTRL_ALT),
        16 => Some(M::SUPER_CTRL_ALT_SHIFT),
        _ => None,
    }
}

fn cursor_key_with_modifier(key: char, mods: M) -> Option<KeyEvent> {
    Some(match key {
        UP => E(K::Up, mods),
        DOWN => E(K::Down, mods),
        ...
        if seq3 == '~' {
            ...
            if seq4.is_ascii_digit() {
                ...
                if seq5.is_ascii_digit() {
                    let seq6 = self.next_char()?;
                    if seq2 == '1' {
                        if let Some(mods) = modifier_from_digits(seq4, Some(seq5)) {
                            if let Some(key) = cursor_key_with_modifier(seq6, mods) {
                                return Ok(key);
                            }
                        }
                    }
                    ...
                    if let Some(mods) = modifier_from_digits(seq4, None) {
                        if let Some(key) = cursor_key_with_modifier(seq5, mods) {
                            return Ok(key);
                        }
                    }
                    Ok(match (seq4, seq5) {
...
mod test {
    use super::{
        cursor_key_with_modifier, modifier_from_digits, AltFd, Position, PosixRenderer,
        PosixTerminal, Renderer as _,
    };
    ...
    #[test]
    fn decodes_super_modifiers_from_csi_digits() {
        assert_eq!(modifier_from_digits('9', None), Some(M::SUPER));
        assert_eq!(modifier_from_digits('1', Some('0')), Some(M::SUPER_SHIFT));
    }

    #[test]
    fn maps_super_shift_cursor_keys() {
        assert_eq!(
            cursor_key_with_modifier('D', M::SUPER_SHIFT),
            Some(E(K::Left, M::SUPER_SHIFT))
        );
        assert_eq!(
            cursor_key_with_modifier('C', M::SUPER_SHIFT),
            Some(E(K::Right, M::SUPER_SHIFT))
        );
    }
}
